...
class Formio {
    ...
    loadForm(query, opts) {
        return this.load('form', query, opts)
            .then(currentForm => {
                ...
                if (!currentForm.revisions || Number.isNaN(parseInt(this.vId, 10))) {
                    return currentForm;
                }
    ...
    saveSubmission(data, opts) {
        if (!Number.isNaN(parseInt(this.vId, 10))) {
            data._fvid = this.vId;
        }
   ...
   */
    ...
    static setUser(user, opts = {}) {
        const userName = `${opts.namespace || getFormio().namespace || 'formio'}User`;
        if (!user) {
            if (!opts.fromToken) {
                opts.fromUser = true;
                getFormio().setToken(null, opts);
    ...
    static getUser(options) {
        options = options || {};
        const userName = `${options.namespace || getFormio().namespace || 'formio'}User`;
        try {
