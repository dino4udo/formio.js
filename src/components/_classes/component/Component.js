...
export default class Component extends Element {
   ...
   */
    getWidgetValueAsString(value, options) {
        ...
        const { widget } = this.refs.input[0];
   ...
   */
    ...
    isIE() {
        ...
        if (msie > 0) {
            ...
            return Number.parseInt(userAgent.substring(msie + 5, userAgent.indexOf('.', msie)), 10);
        }
        ...
        if (trident > 0) {
            ...
            return Number.parseInt(userAgent.substring(rv + 3, userAgent.indexOf('.', rv)), 10);
        }

        const edge = userAgent.indexOf('Edge/');
        if (edge > 0) {
            // IE 12 (aka Edge) => return version number
            return Number.parseInt(userAgent.substring(edge + 5, userAgent.indexOf('.', edge)), 10);
        }
   ...
   */
    ...
    addCKE(element, settings, onChange) {
        ...
        settings.base64Upload = !this.component.isUploadEnabled;
    ...
    addQuill(element, settings, onChange) {
        ...
        return Formio.requireLibrary('quill', 'Quill', _.get(this.options, 'editors.quill.src', `${QUILL_URL}/quill.min.js`), true)
            .then(() => Formio.requireLibrary('quill-table', 'Quill', QUILL_TABLE_URL, true)
                .then(() => {
                    if (!element.parentNode) {
                        return NativePromise.reject();
                    }
